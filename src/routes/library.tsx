...
function LibraryPage() {
  ...
  const onDelete = async (t: Track) => {
    ...
    await supabase.storage.from("audio").remove([t.audio_path]);
    ...
    <AppShell>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {filtered.map((t) => (
            <TrackCard key={t.id} track={t} queue={filtered} onEdit={setEditing} onDelete={onDelete} />
          ))}
        </div>
      )}
...
function EditModal({ track, onClose, onSaved }: { track: Track; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    setBusy(true);
    const { error } = await supabase.from("tracks").update({ title, artist }).eq("id", track.id);
    setBusy(false);
    if (error) return toast.error(error.message);
